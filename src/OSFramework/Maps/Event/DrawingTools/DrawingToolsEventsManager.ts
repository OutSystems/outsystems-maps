				...
				switch (eventType) {
					case DrawingToolsEventType.Initialized:
						handlerEvent.trigger(
							...
							this._drawingTools.widgetId || this._drawingTools.uniqueId, // Id of DrawingTools block that was initialized
							...args
						);
						break;
					case DrawingToolsEventType.ProviderEvent:
						...
						if (this._drawingTools.validateProviderEvent(eventInfo) === true) {
							const handler = this.handlers.get(eventInfo as DrawingToolsEventType);
							handler.trigger(
								this._drawingTools.map.widgetId, // Id of Map block that triggered the event
								eventParams.uniqueId || this._drawingTools.widgetId || this._drawingTools.uniqueId, // Id of marker/shape block (once created by the DrawingTools) that triggered the event
