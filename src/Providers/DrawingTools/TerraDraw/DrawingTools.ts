...
namespace Provider.DrawingTools.TerraDraw {
	...
	> {
		...
		constructor(map: OSFramework.Maps.OSMap.IMap, drawingToolsId: string, configs: JSON) {
			super(map, drawingToolsId, new Provider.Maps.Google.Configuration.DrawingTools.DrawingToolsConfig(configs));
		...
		private _buildTerraDraw(): void {
			...
			const adapter = new globalThis.terraDrawGoogleMapsAdapter.TerraDrawGoogleMapsAdapter({
			...
			this.tools.forEach((tool) => {
				...
				this._modeToTool.set(t.terraDrawModeName, t);
			});
		}
		...
		public addTool(tool: OSFramework.Maps.DrawingTools.ITool): OSFramework.Maps.DrawingTools.ITool {
			...
			if (this.isReady) {
				...
				const modeNames = this.tools.map(
					(t) =>
						(t as AbstractProviderTool<OSFramework.Maps.Configuration.IConfigurationTool>).terraDrawModeName
				);
		...
		public build(): void {
			...
			if (mapContainer) {
				this._ui = new DrawingToolsUi(
					mapContainer,
					(modeName) => this._provider.setMode(modeName),
					() => this._provider.setMode('select')
				);
				const modeNames = this.tools.map(
					(t) =>
		...
		public changeProperty(propertyName: string, value: unknown): void {
			...
			if (this.isReady) {
				if (propValue === OSFramework.Maps.Enum.OS_Config_DrawingTools.position) {
					const modeNames = this.tools.map(
						(t) =>
							(t as AbstractProviderTool<OSFramework.Maps.Configuration.IConfigurationTool>)
								.terraDrawModeName
					);
