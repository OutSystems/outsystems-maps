...
namespace Provider.Maps.Google.FileLayer {
	...
	> {
		...
		private _setFileLayerEvents(): void {
			...
			// OnClick Event
			if (
				this.fileLayerEvents.hasHandlers(OSFramework.Maps.Event.FileLayer.FileLayersEventType.OnClick) &&
				this.provider.get('clickable') // Always true. Fallback in case this parameter gets changed in the future.
			) {
				this.provider.addListener('click', (event: google.maps.KmlMouseEvent) => {
					const flParams: OSFramework.Maps.FileLayer.IFileLayerEventParams = {
						...
						featureData: JSON.stringify(event.featureData as google.maps.KmlFeatureData),
					};
