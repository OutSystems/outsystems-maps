...
namespace Provider.Maps.Google.OSMap {
	...
	export class StaticMap
		extends OSFramework.Maps.OSMap.AbstractMap<google.maps.Map, Configuration.OSMap.GoogleStaticMapConfig>
		implements IMapGoogle
	{
		...
		private _getCenter(center?: string): string {
			...
			let position = center === undefined ? (this.config.center as string) : center;
		...
		private _renderImage(): void {
			const container = OSFramework.Maps.Helper.GetElementByUniqueId(this.uniqueId);
			const image = container.querySelector(OSFramework.Maps.Helper.Constants.staticMapCss) as HTMLImageElement;
		...
		public changeProperty(propertyName: string, propertyValue: unknown): void {
			...
			switch (propValue) {
				case OSFramework.Maps.Enum.OS_Config_StaticMap.apiKey:
					if (this.config.apiKey !== '') {
						this.mapEvents.trigger(
							OSFramework.Maps.Event.OSMap.MapEventType.OnError,
							this,
							OSFramework.Maps.Enum.ErrorCodes.CFG_APIKeyAlreadySetStaticMap
						);
					}
				...
				case OSFramework.Maps.Enum.OS_Config_StaticMap.type:
					this._type = propertyValue as OSFramework.Maps.Enum.OSMap.Type;
					return;
				default:
					this.mapEvents.trigger(
						OSFramework.Maps.Event.OSMap.MapEventType.OnError,
