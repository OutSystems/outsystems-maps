...
namespace Provider.Maps.Google.SearchPlaces {
	export class SearchPlacesV2 extends OSFramework.Maps.SearchPlaces.AbstractSearchPlaces<
	...
	> {
		...
		constructor(searchPlacesId: string, configs: JSON) {
			super(searchPlacesId, new Configuration.SearchPlaces.SearchPlacesConfigV2(configs));
			this._addedEvents = [];
		...
		private _createProvider(configs: Configuration.SearchPlaces.ISearchPlacesProviderConfigV2): void {
		...
		private _prepareProviderConfigs(isModuleAvailable: boolean): void {
			if (isModuleAvailable) {
				const local_configs =
					this.getProviderConfig() as Configuration.SearchPlaces.ISearchPlacesProviderConfigV2;
		...
		private _setSearchPlacesEvents(): void {
			...
			if (
				...
				this._addedEvents.push(Constants.SearchPlaces.EventsV2.OnPlaceSelect);
				this._provider.addEventListener(
					Constants.SearchPlaces.EventsV2.OnPlaceSelect,
					...
					async ({ placePrediction }: any) => {
						...
						this.searchPlacesEvents.trigger(
							...
							Constants.SearchPlaces.EventsV2.OnPlaceSelect, // event name (eventInfo)
