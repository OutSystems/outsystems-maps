...
namespace Provider.Maps.Google.Feature {
	...
	export abstract class AbstractFactoryBuilder implements IFeatures, OSFramework.Maps.Interface.IBuilder {
		...
		public dispose(): void {
			this._featureList.forEach((p) => {
				this._instanceOfIDisposable(p) && (p as OSFramework.Maps.Interface.IDisposable).dispose();
				p = undefined;
			});
		}
	}
	...
	export class FeatureBuilder extends AbstractFactoryBuilder {
		private _makeCenter(center: OSFramework.Maps.OSStructures.OSMap.Coordinates): FeatureBuilder {
		...
		private _makeMarkerClusterer(
			...
			this._features.markerClusterer = this._makeItem(GoogleMarkerClusterer, markerClusterer);
			return this;
		}
		private _makeOffset(offset: OSFramework.Maps.OSStructures.OSMap.Offset): FeatureBuilder {
		...
		public build(): void {
			const config = this._map.config as Configuration.OSMap.GoogleMapConfig;

			this._makeShape();
			this._makeTrafficLayer(config.showTraffic)
				._makeZoom(config.zoom)
				._makeCenter(config.center as OSFramework.Maps.OSStructures.OSMap.Coordinates)
				._makeDirections()
