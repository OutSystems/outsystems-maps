...
namespace Provider.Maps.Leaflet.Shape {
	...
	> extends OSFramework.Maps.Shape.AbstractShape<W, T> {
		...
		private _setDragEditConfigs(allowDrag: boolean, allowEdit: boolean): void {
		...
		protected setShapeEvents(): void {
			...
			this.shapeEvents.handlers.forEach((handler: OSFramework.Maps.Event.IEvent<string>, eventName: string) => {
				if (handler instanceof OSFramework.Maps.Event.Shape.ShapeProviderEvent) {
					...
					if (eventName === OSFramework.Maps.Helper.Constants.shapeChangedEvent) {
						...
						this.providerEventsList.forEach((event) =>
							this.provider.addEventListener(event, () => {
								...
								this._shapeChangedEventTimeout = setTimeout(
									this.triggerShapeChangedEvent.bind(this),
									500
								);
							})
						);
						...
						Constants.Shape.ProviderSpecialEvents.indexOf(eventName) !== -1
					) {
			...
			if (this.isReady) {
				switch (propValue) {
					case OSFramework.Maps.Enum.OS_Config_Shape.allowDrag:
						this._setDragEditConfigs(propertyValue as boolean, this.config.allowEdit);
						return;
					case OSFramework.Maps.Enum.OS_Config_Shape.allowEdit:
						this._setDragEditConfigs(this.config.allowDrag, propertyValue as boolean);
						return;
