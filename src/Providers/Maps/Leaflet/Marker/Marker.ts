...
namespace Provider.Maps.Leaflet.Marker {
	export class Marker
		extends OSFramework.Maps.Marker.AbstractMarker<L.Marker, Configuration.Marker.LeafletMarkerConfig>
		implements IMarkerLeaflet
	{
		...
		constructor(
			...
			super(map, markerId, type, new Configuration.Marker.LeafletMarkerConfig(configs));
		...
		private async _getMeta(url: string): Promise<{ height: number; width: number }> {
					...
					this._provider.addEventListener(
						...
						(e?: L.LeafletMouseEvent) => {
							this.markerEvents.trigger(
								...
								eventName,
								// Coords
								e !== undefined && e.target.getLatLng !== undefined
									? JSON.stringify({
			...
			if (this.isReady) {
				switch (property) {
					...
					case OSFramework.Maps.Enum.OS_Config_Marker.title:
						this._provider.getElement().title = propertyValue as string;
						return;
				}
			}
		}
