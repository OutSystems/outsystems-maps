...
namespace MapAPI.Directions {
	...
	export function LoadPlugin(mapId: string, providerName: string, apiKey: string): string {
		OSFramework.Maps.Helper.LogWarningMessage(
	...
	export function SetDirections(mapId: string, options: string): Promise<string> {
		OSFramework.Maps.Helper.LogWarningMessage(
